module.exports = function(app, passport) {
    ...
    .get('/myprofile', passport.authenticate('basic', {
        session: false
        ...
        // change themesetting
        .put('/updateprofile', passport.authenticate('basic', {
            session: false
        }), function(req, res, next) {
        ...
        .delete('/deleteprofile', passport.authenticate('basic', {
            session: false
        }), function(req, res, next) {
        ...
        .post('/signup', function(req, res, next) {
            User.create(req.body, function(err, post) {
                ...
                email.setHtml(signup);
                email.addSubstitution("%username%", post.firstname);
                sendgrid.send(email);
            });
        })
        // get a user (accessed at GET http://localhost:8080/getuserbyemail/:email
            .get('/getuserbyemail/:email', function(req, res, next) {
                User.findOne({}).where('email').equals(req.params.email).exec(function(err, user) {
                    if (err) {
                        return res.status(404).send("Not Found");
                    }
                    if(user != null){
                        console.log("getUserbyemail success",user);
        ...
        /* STATE
        -------------------------------------*/
        .post('/addstatetouser', passport.authenticate('basic', {
            session: false
        }), function(req, res, next) {
        ...
        /* MESSAGE
        -------------------------------------*/
        .get('/messagebytype/:type', passport.authenticate('basic', {
            session: false
        }), function(req, res, next) {
            console.log("req.params.type", req.params.type);
            Message.find({}).where('type').equals(req.params.type).exec(function(err, messages) {
                if (err) return next(err);
        ...
        /* NOTIFICATION
        -------------------------------------*/
        .post('/addnotificationtouser', passport.authenticate('basic', {
            session: false
        }), function(req, res, next) {
        ...
        }).get('/randomnotification', passport.authenticate('basic', {
            session: false
        }), function(req, res, next) {
        ...
        }).get('/notificationsbycategory/:catid', passport.authenticate('basic', {
            session: false
        }), function(req, res, next) {
            console.log("req.params.catid", req.params.catid);
            console.log("req.params.type", req.params.type);
            Notification.find({}).where('category').equals(req.params.catid).populate('category').exec(function(err, notifications) {
                if (err) return next(err);
        ...
        }).get('/notificationsbytype/:type', passport.authenticate('basic', {
            session: false
        }), function(req, res, next) {
            console.log("req.params.catid", req.params.catid);
            console.log("req.params.type", req.params.type);
            Notification.find({}).where('type').equals(req.params.type).populate('category').exec(function(err, notifications) {
                if (err) return next(err);
                console.log(notifications);
                console.log('count', notifications.length);
                var randomNumber = Math.floor((Math.random() * (notifications.length)));
        ...
        }).get('/notificationsbycategory/:catid/type/:type', passport.authenticate('basic', {
            session: false
        }), function(req, res, next) {
            console.log("req.params.catid", req.params.catid);
            console.log("req.params.type", req.params.type);
            Notification.find({}).where('category').equals(req.params.catid).where('type').equals(req.params.type).populate('category').exec(function(
                err, notifications) {
                if (err) return next(err);
                //console.log(notifications);
                //console.log('count', notifications.length);
                ...
                do{
                    var randomNumber = Math.floor((Math.random() * (notifications.length)));
                    console.log('randomNumber', randomNumber);    
                    console.log('randomnotification',notifications[randomNumber]);
                    console.log('user notifications',req.user.notifications);
                    counter++;
                }
                while(req.user.notifications.indexOf(notifications[randomNumber].id) != -1 || counter < 10) //as long as the id of the random chosen note is already given to the user
                    res.json(notifications[randomNumber]);
            });
        }).get('/reminder', passport.authenticate('basic', {
            session: false
        }), function(req, res, next) {
            if (req.user.role === "admin") {
                User.find({}).populate('states').populate('states.category').exec(function(err, users) {
                    if (err) return next(err);
                    async.forEach(users, function(user, callback) {
                        if (user.role !== "admin") {
                            ...
                            if (diffDays >= 0) {
                                State.populate(user.states[0], {
                                    "path": "category"
                                }, function(err, output) {
                                    if (err) {
                                        email.addSubstitution("%feeling%", "like a unicorn");
                                        console.log("error",err);
                                        throw err;
                                    }else{
                                        email.addSubstitution("%feeling%", output.category.name);
                                    }
                                    console.log("output", output);
                                    email.addTo(user.email);
                                    email.setFrom("cosycare.app@gmail.com");
                                    email.setSubject("Reminder from Cosy Care!");
                                    email.setHtml(reminder);
                                    email.addSubstitution("%username%", user.firstname);
                                    // email.addSubstitution("%createdat%", user.states[0].created_at);
